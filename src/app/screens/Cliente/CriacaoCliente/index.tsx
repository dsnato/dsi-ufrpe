...
const CriarCliente: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (name.trim().length < 3) {
      showError(t("validation.nameMinLength"));
      return;
    }
    ...
    if (!phoneRegex.test(phone)) {
      showError(t("validation.invalidPhoneFormat"));
      return;
    }
    ...
    try {
      ...
      console.log(
        "📤 [CriacaoCliente] Enviando dados:",
        JSON.stringify(novoCliente, null, 2)
      );
      ...
      if (imagemUri && resultado.id) {
        try {
          ...
          showError(t("validation.clientCreatedImageError"));
        }
      }
      ...
      console.error(
        "❌ [CriacaoCliente] Detalhes do erro:",
        JSON.stringify(error, null, 2)
      );
      showError(t("validation.clientCreationError"));
    } finally {
      setLoading(false);
          ...
          <View style={styles.form}>
            ...
            <View style={[styles.fieldGroup, styles.halfWidth]}>
              ...
              <FormInput
                ...
                placeholder={t("clients.streetName")}
                value={street}
            ...
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("clients.neighborhood")}
              </Text>
              <FormInput
                icon="home-outline"
                placeholder={t("clients.neighborhoodName")}
                value={neighborhood}
            ...
            <View style={[styles.fieldGroup, styles.halfWidth]}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("clients.city")} <Text style={styles.required}>*</Text>
              </Text>
              <FormInput
                icon="business-outline"
                placeholder={t("clients.cityName")}
                value={city}
            ...
            <View style={[styles.fieldGroup, styles.halfWidth]}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("clients.state")} <Text style={styles.required}>*</Text>
              </Text>
              <FormSelect
                icon="map-outline"
                placeholder={t("clients.selectState")}
                value={state}
                options={estadosBrasileiros}
                onSelect={setState}
                disabled={loading}
                helperText={t("clients.stateUF")}
                isDarkMode={isDarkMode}
              />
            </View>
            {/* <View style={styles.row}>
          ...
          <View style={styles.actions}>
            ...
            >
              {loading
                ...
                : t("clients.createClient", "Criar Cliente")}
            </ActionButton>
