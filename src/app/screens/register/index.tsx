...
export default function RegisterScreen() {
  ...
  const handleCnpjChange = (text: string) => {
    ...
    if (limited.length >= 9) {
      formatted = `${limited.slice(0, 2)}.${limited.slice(
        2,
        5
      )}.${limited.slice(5, 8)}/${limited.slice(8)}`;
    }
    if (limited.length >= 13) {
      formatted = `${limited.slice(0, 2)}.${limited.slice(
        2,
        5
      )}.${limited.slice(5, 8)}/${limited.slice(8, 12)}-${limited.slice(12)}`;
    }
  ...
  async function signUpWithEmail() {
    ...
    if (error) {
      ...
      showInfo(
        "📧 Verifique seu e-mail para confirmar o cadastro e ativar sua conta!"
      );
    } else {
  ...
  const handleSubmitRegister = () => {
    ...
    if (!confirmPassword) {
      showError(t("validation.confirmPassword"));
      return;
    }
      ...
      >
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t("profile.mainData")}</Text>
          </View>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t("profile.hotelName")}</Text>
          </View>
          ...
          <View style={styles.inputGroup}>
            ...
            <FormInput
              ...
              placeholder={`${t("profile.cnpj")} *`}
              value={cnpj}
              onChangeText={handleCnpjChange}
              keyboardType="numeric"
              helperText={t("validation.cnpjFormat")}
              maxLength={18}
            />
            ...
            <FormInput
              ...
              helperText={t("validation.phoneFormat")}
              maxLength={15}
            />
          </View>
        </View>
        ...
        <View style={styles.actions}>
          <ActionButton
            ...
            disabled={loading}
          >
            {loading ? t("profile.saving") : t("auth.register")}
          </ActionButton>
