...
const CriarReserva: React.FC = () => {
  ...
  const calcularValorTotal = useCallback(() => {
    ...
    const quartoSelecionado = quartosData.find(
      (q) => q.numero_quarto === numeroQuarto
    );
  ...
  const carregarQuartosDisponiveis = useCallback(async () => {
    try {
      ...
      const options: SelectOption[] = quartosDisponiveisLista.map((q) => ({
        label: `Quarto ${q.numero_quarto} - ${q.tipo} (R$ ${q.preco_diario
          ?.toFixed(2)
          .replace(".", ",")}/dia)`,
      ...
      showError(
        t(
          ...
          "Não foi possível carregar os quartos disponíveis."
        )
      );
    } finally {
  ...
  const carregarClientesDisponiveis = useCallback(async () => {
    try {
      ...
      if (options.length === 0) {
        showError(
          t(
            "validation.noClientsRegistered",
      ...
      showError(
        t("messages.loadClientsError", "Não foi possível carregar os clientes.")
      );
    } finally {
  ...
  const handleSave = async () => {
    ...
    if (!clienteId.trim()) {
      showError(
        t("validation.selectClient", "Selecione um cliente para a reserva.")
      );
      return;
    }
    ...
    if (!checkInDate) {
      showError(
        t(
          "validation.invalidCheckInDate",
          "Data de check-in inválida. Use o formato DD/MM/AAAA."
        )
      );
      return;
    }
    ...
    if (checkInDate > maxFutureDate) {
      showError(
        t(
          "validation.checkInTooFarFuture",
    ...
    if (!checkOutDate) {
      showError(
        t(
          "validation.invalidCheckOutDate",
    ...
    if (checkOutDate <= checkInDate) {
      showError(
        t(
          "validation.checkOutBeforeCheckIn",
    ...
    if (diffDays > 365) {
      showError(
        t(
          "validation.reservationTooLong",
          "A reserva não pode ter duração superior a 365 dias."
        )
      );
      return;
    }
    ...
    if (valorNumerico <= 0) {
      showError(
        t(
          "validation.cannotCalculateTotal",
    ...
    try {
      ...
      const quarto = quartos.find(
        (q) => q.numero_quarto === numeroQuarto.trim()
      );

      if (!quarto) {
        showError(
          t(
            "validation.roomNotFound",
            "Quarto não encontrado. Verifique o número informado."
          )
        );
        setLoading(false);
        return;
      ...
      showError(
        t(
          "messages.createReservationError",
        ...
        >
          ...
          <View style={styles.titleContainer}>
            ...
            <Text style={[styles.title, { color: theme.text }]}>
              {t("reservations.newReservation", "Nova Reserva")}
            </Text>
          </View>
          ...
          <View style={styles.form}>
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("reservations.roomNumber", "Número do Quarto")}{" "}
              ...
              <FormSelect
                ...
                helperText={t(
                  "reservations.availableRoomsCount",
                  "{{count}} quarto(s) disponível(is)",
              ...
              <FormSelect
                ...
                placeholder={
                  loadingClientes
                    ...
                    : t("reservations.selectClient", "Selecione um cliente")
                }
                value={clienteId}
                ...
                helperText={t(
                  "reservations.registeredClientsCount",
                  "{{count}} cliente(s) cadastrado(s)",
            ...
            <View style={[styles.fieldGroup, styles.halfWidth]}>
              ...
              <FormInput
                ...
                isDarkMode={isDarkMode}
                maxLength={10}
                helperText={t("reservations.checkInDate", "Data de entrada")}
              />
            </View>

            <View style={[styles.fieldGroup, styles.halfWidth]}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("reservations.checkOut", "Check-out")}{" "}
                <Text style={styles.required}>*</Text>
              </Text>
              <FormInput
                ...
                keyboardType="numeric"
                maxLength={10}
                helperText={t("reservations.checkOutDate", "Data de saída")}
                isDarkMode={isDarkMode}
              />
            </View>
            ...
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("reservations.totalValue", "Valor Total")}
              </Text>
              <View
                style={[
                  styles.valorTotalContainer,
                  {
              ...
              >
                ...
                <View style={styles.valorTotalContent}>
                  <Text
                    style={[
                      styles.valorTotalLabel,
                      {
                        color: isDarkMode ? "#6EE7B7" : "#059669",
                      },
                    ]}
                  >
                    {t("reservations.reservationTotal", "Total da Reserva")}
                  </Text>
                  <Text
                    style={[
                      styles.valorTotalValue,
                      {
                  ...
                  {calcularValorTotal() > 0 && (
                    ...
                    >
                      {t(
                        "reservations.calculatedAutomatically",
            ...
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: theme.text }]}>
                {t("reservations.notes", "Observações")}
              </Text>
              <FormInput
                icon="document-text-outline"
                placeholder={t(
                  "reservations.additionalInfo",
              ...
              <View style={styles.switchLabel}>
                ...
                <View style={styles.switchTextContainer}>
                  ...
                  >
                    {confirmada
                      ? t(
                          "reservations.clientConfirmed",
                          "Cliente confirmou a reserva"
                        )
                      : t(
                          "reservations.awaitingConfirmation",
          ...
          <View style={styles.actions}>
            ...
            >
              {loading
                ...
                : t("reservations.createReservation", "Criar Reserva")}
            </ActionButton>
